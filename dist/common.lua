...
function M.exchange_token_for_user_info(token)
    print("exchange_token_for_user_info")
...
function M.exchange_token_for_full_user_info(token)
    print("inside exchange_token_for_full_user_info")
    ngx.var.user_token = 'Bearer '..token;
    print(ngx.var.user_token);
    local response = ngx.location.capture(
        '/full/_full_userinfo',
        {
            method = ngx.HTTP_GET
...
function get_current_user()
    ...
    local full_user_info = M.get_value(session.id..'_full_user_info')
    ...
    local full_user_info_json = full_user_info

    user_info_table['user_name'] = user_info_json["user_name"]
    user_info_table['email'] = user_info_json["email"]
    user_info_table['full_user_info'] = full_user_info_json
    user_info_table['token'] = token
    return user_info_table;
end
...
function M.get_current_full_user_info()
    return cjson.encode(get_current_user())
end
